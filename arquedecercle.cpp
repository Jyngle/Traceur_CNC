...
/*
ArqueDeCercle::ArqueDeCercle(float _X, float _Y, float _Z, float _I, float _J, float _F){
...
QList<float> ArqueDeCercle::get_info_abs(){
    QList<float> liste;
...
QList<float> ArqueDeCercle::get_info_rel(){
    QList<float> liste;
    //qDebug() << "X = " <<  QString::number(X) << "Y = " << QString::number(Y) << "I = " << QString::number(I) << "J = " << QString::number(J) << "F = " << QString::number(F);
    liste.append(X_rel);
    liste.append(Y_rel);
...
float ArqueDeCercle::get_distance(){
